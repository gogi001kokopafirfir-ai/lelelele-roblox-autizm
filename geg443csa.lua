-- visual-deer-morph-fixed10.lua  (патч)
-- Исправления: не ломаем CanCollide реального чара; не анкерим визуал; snap+zero velocity each frame.
...
local function bindTools(container)
    ...
    for _, t in ipairs(container:GetChildren()) do
        if t:IsA("Tool") and not toolConns[t] then
            toolConns[t] = {}
            toolConns[t].equip = t.Equipped:Connect(function() setLocalVisibility(t, false, true) end)
            toolConns[t].unequip = t.Unequipped:Connect(function() setLocalVisibility(t, true, true) end)
    ...
    if not childAddedConns[container] then
        childAddedConns[container] = container.ChildAdded:Connect(function(child)
            if child:IsA("Tool") and not toolConns[child] then
                toolConns[child] = {}
                toolConns[child].equip = child.Equipped:Connect(function() setLocalVisibility(child, false, true) end)
                toolConns[child].unequip = child.Unequipped:Connect(function() setLocalVisibility(child, true, true) end)
